# Oak MCP — контракт для агента (вариант Б: Go)
...
`oak-mcp` — **отдельный бинарник** из `cmd/oak-mcp`, поднимаемый Claude Code по stdio. Он не общается с приложением: он импортирует те же `internal/vault` и `internal/index` и работает с vault напрямую.

```
Claude Code ──stdio──> oak-mcp ──> internal/vault ──> ~/Notes/*.md
                                └─> internal/index ──> .oak/index.sqlite
                                                             ↑
                                 oak.app (fsnotify) ─────────┘  видит изменения,
                                                                шлёт oak:notes-changed
```
...
Отдельный бонус Go: `oak-mcp` — **один статический бинарник без зависимостей**. Ни рантайма, ни `node_modules`, ни путей до интерпретатора. Скопировал файл — работает.
...
```sh
go build -o ~/bin/oak-mcp ./cmd/oak-mcp
claude mcp add oak --scope user -- ~/bin/oak-mcp --vault ~/Notes
```
...
Положить в `~/.claude/skills/oak-notes/SKILL.md`.
...
Ссылки на заметки — `[Заголовок](oak://note/<ULID>)`. Wiki-ссылки не поддерживаются.
...
5. **Приложение открыто, агент создаёт вторую заметку** — она появляется в списке без перезапуска, через `oak:notes-changed`.
