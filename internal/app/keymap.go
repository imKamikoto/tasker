...
func defaultKeymap() map[string]map[string]string {
	return map[string]map[string]string{
		ContextGlobal: {
			...
			// Те же команды на стрелках. Не удобство, а обязательство: буквы
			// здесь — вимовское движение, и при выключенных движениях они
			// снимаются. Без стрелок выйти из колонки клавиатурой стало бы
			// нечем, а этого не должно случаться ни при каких настройках.
			// Везде остальное буква и стрелка соседствуют с самого начала
			// (j и down в списке), здесь эта пара просто опоздала.
